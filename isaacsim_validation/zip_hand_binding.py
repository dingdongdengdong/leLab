...
try:
    from .prepare_amazinghand_usd import DISTAL_REFERENCES, PROXIMAL_REFERENCES
...
def visual_reference_contract(prepared_package: Path) -> dict:
    ...
    return {
        ...
        "proximal": list(PROXIMAL_REFERENCES),
        "distal": list(DISTAL_REFERENCES),
        "finger_count": 4,
        "static_visual_part_count": 26,
        "moving_visual_part_count": 16,
    }
...
{{
    ...
    uniform token[] xformOpOrder = ["xformOp:translate"]
        ...
        for index, source_name in enumerate(DISTAL_REFERENCES, start=1)
    )
...
''',
    ...
    for finger in range(1, 5):
        ...
        for index, source_name in enumerate(PROXIMAL_REFERENCES, start=1):
            prim = UsdGeom.Xform.Define(
                stage,
            ...
            prim.SetInstanceable(True)
        for index, _source_name in enumerate(DISTAL_REFERENCES, start=1):
            prim = UsdGeom.Xform.Define(
                stage,
