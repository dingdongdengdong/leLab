...
tags: ["so101", "lerobot", "superarm", "mujoco", "amazinghand", "recording", "calibration"]
created: 2026-07-21T14:48:57.876Z
updated: 2026-07-21T15:08:02.887Z
sources: []
links: []
category: pattern
confidence: medium
...
## Update (2026-07-21T15:08:02.887Z)

## Contract
Use LeLab's existing physical `SO101Leader` recording path with the SuperArm MuJoCo follower first. The Manual Web Leader page is browser sliders only, not a physical SO-101.

## Follower identity and five-joint calibration
The follower is `SuperArm DM4340P + AmazingHand`; its first target is `MuJoCo SuperArm + AmazingHand`.

Before real DM4340P torque is enabled, copy `lelab/superarm/data/superarm_dm4340p_amazinghand.example.yaml` outside the repository and replace every intentionally invalid placeholder. The real SuperArm needs calibration for exactly `joint_rev_1` through `joint_rev_5`. Each calibration is `[direction, zero_offset_rad]`; also measure five joint limits and five `position_kp` and `position_kd` values. This is separate from SO-101 leader calibration.
