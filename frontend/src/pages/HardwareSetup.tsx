  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <main className="mx-auto max-w-6xl space-y-5 p-4">
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900 p-5"><h2 className="font-semibold">Configuration handoff</h2><p className="mt-1 text-sm text-slate-400">Copy the template path, then replace all intentionally invalid placeholders from real discovery and calibration. Do not use OpenArm sample IDs.</p><div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg bg-slate-950 p-3 font-mono text-xs text-slate-300"><code className="min-w-0 flex-1 break-all">{readiness?.config_template ?? "Loading…"}</code><Button size="sm" variant="outline" onClick={() => void copyTemplate()} disabled={!readiness}><Copy className="mr-2 h-3 w-3" />Copy</Button></div><Button className="mt-4" variant="outline" onClick={() => navigate("/so101-leader-setup")}>Open SO-101 leader guide</Button></section>
